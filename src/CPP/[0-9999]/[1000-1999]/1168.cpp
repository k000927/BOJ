...
int query(int node, int s, int e, int order){
	...
	if(order <= seg[node*2]) return query(node*2, s, mid, order);
	else return query(node*2+1, mid+1, e, order-seg[node*2]);
}
...
int update(int node, int s, int e, int del){
	...
	if(s==e) return 0;
		...
		int mid = (s+e)/2;
		if(del<=mid) return update(node*2, s, mid, del);
		else return update(node*2+1, mid+1, e, del);
	}
}

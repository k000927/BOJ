...
public class Main {
  ...
  public static void main(String args[]) throws IOException {
    ...
    // true = java, false = Cpp
    if (whatIsLanguage) sb = javaToCpp(sb); else sb = cppToJava(sb);
  ...
  public static boolean name_check(StringBuilder sb) {
    ...
    for (int i = 0; i < sb.length(); i++) {
      ...
      if (ch >= 'A' && ch <= 'Z') isUpperCase = 1; else if (
        ch == '_'
      ) isUnderBar = 1;
    }
  ...
  public static boolean is_Cpp(StringBuilder sb) {
    if (sb.charAt(sb.length() - 1) == '_') return false; else if (
      sb.charAt(0) == '_'
    ) return false; else if (sb.indexOf("__") >= 0) return false;
